...
class BSDESolver(object):
    ...
    def __init__(self, config, bsde):
        ...
        lr_schedule     = tf.keras.optimizers.schedules.PiecewiseConstantDecay( self.net_config.lr_boundaries, self.net_config.lr_values)
    ...
    def train(self):
        ...
        for step in range(self.net_config.num_iterations+1):
            ...
            if step % self.net_config.logging_frequency == 0:
                ...
                if self.net_config.verbose:  logging.info("step: %5u,    loss: %.4e, Y0: %.4e,   elapsed time: %3u" % ( step, loss, y_init, elapsed_time))
    ...
    def loss(self, inputs, training):
        ...
        loss       = tf.reduce_mean(tf.where(tf.abs(delta) < DELTA_CLIP, tf.square(delta),   # use linear approximation outside the clipped range
...
class NonsharedModel(tf.keras.Model):
    def __init__(self, config, bsde):
        ...
        self.y_init     = tf.Variable(np.random.uniform(low=self.net_config.y_init_range[0],
                                                        high=self.net_config.y_init_range[1],    size=[1]) )
        self.z_init     = tf.Variable(np.random.uniform(low=-.1, high=.1, size=[1, self.eqn_config.dim])                                  )
...
class FeedForwardSubNet(tf.keras.Model):
    def __init__(self, config):
        ...
        self.bn_layers    =  [tf.keras.layers.BatchNormalization(  momentum=0.99,  epsilon=1e-6,
                              beta_initializer=tf.random_normal_initializer(0.0, stddev=0.1),  gamma_initializer=tf.random_uniform_initializer(0.1, 0.5)
                              ) for _ in range(len(num_hiddens) + 2)]
        
        self.dense_layers = [tf.keras.layers.Dense(num_hiddens[i],   use_bias=False,    activation=None)
